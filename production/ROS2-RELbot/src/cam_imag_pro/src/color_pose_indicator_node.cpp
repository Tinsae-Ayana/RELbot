...
ColorPoseNode::ColorPoseNode() : Node("color_pose_indicator_node") {
  pose_publisher = this->create_publisher<geometry_msgs::msg::PointStamped>(
      "colored_shape_pose", 1);
  publish_with_mark =
...
void ColorPoseNode::callback(const sensor_msgs::msg::Image &cam_frame) {
  ...
  if (m.m00 > 0) {
    ...
    // get the center pixel of the image from camera on the Relbot
    int center_x =
        frame.cols / 2; // sometimes for some reason this gets to zero

    if (center_x != 0) { // update the center only when center_x is not zero
      this->frame_center = center_x;
    }
    ...
    center_of_gravity.point.z = this->frame_center;
