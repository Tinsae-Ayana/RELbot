...
NativeBase::~NativeBase() { printf("%s: Destructing rampio\n", __FUNCTION__); }
...
void NativeBase::init() {
  ...
  rfd = evl_create_xbuf(0, sizeof(RosData) * ROS2XENO_XBUF_SIZE,
                        EVL_CLONE_PUBLIC | EVL_CLONE_NONBLOCK, "Ros-Xeno");
  if (rfd < 0)
  ...
  xfd = evl_create_xbuf(sizeof(XenoData) * XENO2ROS_XBUF_SIZE, 0,
                        EVL_CLONE_PUBLIC, "Xeno-Ros");
  if (xfd < 0)
...
// Check if kill condition has been met
    ...
    while (true) {
      ...
      if ((sleepTime.tv_sec < timeout.tv_sec) ||
          ((sleepTime.tv_sec == timeout.tv_sec) &&
           (sleepTime.tv_nsec < timeout.tv_nsec)))
        break;
